...
import {StyleSheet, Text, FlatList, View, Image, TouchableOpacity, TextInput, ActivityIndicator} from 'react-native';
...
export default class ContactFlatList extends Component<Props> {
    ...
    _renderContactsItem = ({item, index}) => {
        return(
            <TouchableOpacity
                onPress = {() => this._goDetailContact(item)}
                style = {[styles._itemContainer, {backgroundColor: index % 2 === 0 ? '#fafafa' : ''}]}>
                <Image
